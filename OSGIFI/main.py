...
def main():
    ...
    if infos["user"]==None:
        ...
        print("Account Verified       : "+str(infos['is_verified'])+" | Is buisness Account : "+str(infos["is_business"]))
        print("Is private Account     : "+str(infos["is_private"]))
        print("Number of Follower     : "+str(infos["follower_count"]) + " | Following : "+str(infos["following_count"]))
        print("Number of posts        : "+str(infos["media_count"]))
        print("Number of tag in posts : "+str(infos["following_tag_count"]))
        print("Website URL           : "+infos["website_url"])
        ...
        if "public_email" in infos.keys():
            if infos["public_email"]!='':
                print("Public E-mail           : "+infos["public_email"])
            else:
                print("We Don't Find Any Public Email  : ")
        ...
        if other_infos["error"]=="rate limit":
            ...
            if "hidden_email" in other_infos["user"].keys():
                if other_infos["user"]["hidden_email"]!='':
                    print("Hidden E-mail       : "+other_infos["user"]["hidden_email"])
                else:
                    print("We Don't Find Hidden E-mail")

            if "hidden_number"in other_infos["user"].keys():
                if str(other_infos["user"]["hidden_number"])!='':
                    print("Hidden Number       : "+str(other_infos["user"]["hidden_number"]))
                else:
                    print("We Don't Find Any Hidden Number")
        print("-"*24)
